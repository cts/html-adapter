...
Util._.extend(HtmlNode.prototype, Model.Node.Base, Util.Events, HtmlNodeBase, {
  ...
  _setup_triggers: function() {
    var self = this;
    /* This is a temporary implementation of events */
    if (this.value.attr('on-click')) {
      if (this.value.attr('on-click').trim().toLowerCase() == 'delete') {
        // Delete 
        this.value.on('click', function(evt) {
          self._trigger('click', 'delete', evt);
        })
      }
    }
  },

  _trigger: function(triggerName, actionName, evtObj) {
    // Deletes the enumerable.
    if (actionName == 'delete') {
      this.deleteFromNearestEnumeration();
    }
  },
